...
const llDesc = 
<div>
	<p>The autopilot for my lunar lander game was trained using an <a href="https://en.wikipedia.org/wiki/Evolutionary_algorithm" target="blank">evolutionary algorithm</a>. Each member of a population is scored based on whatever fitness function the framework is provided, and the highest scoring members create children who inherit the "genes" (here, neural network parameters) of the parents with a small chance of mutation. Over several generations, the fitness of the group trends upward. Supervised training (e.g. gradient descent) is not possible for problems like this since it is unknowable how a small change in parameter will effect the overall performance. Evolutionary algorithms have <a href="https://projecteuler.net/problem=185" target="blank">diverse applications</a>.</p>

	<p>Try to land as well as the autopilot! Use the arrow keys to control the lunar lander with autopilot disabled.</p>

	<p> Score is computed by (100 - Impact Speed) + (0.05 * Remaining Fuel).</p>
</div>
...
const drDesc = 
<div>
	<p>This digit recognizer was trained using data from <a href="http://yann.lecun.com/exdb/mnist/" target="blank">The MNIST Database</a>. The above image shows how a convolutional neural network is specified in my framework. The system will then attempt to train the network using gradient descent to classify whatever problem it is given. Gradients for each kind of layer were computed by hand, and the back propagation was written from scratch. <a href="https://aparapi.com/" target="blank">Aparapi</a> was used to parallelize the training with available hardware.</p>

	<p>Click and drag to draw, and move slowly for a denser line. Feel free to sylize in your own script, though the recognizer works best if digits are scaled and centered similarly to the training data. The network used on this website achieves a score of 97% accuracy on training and test data.
	</p>
</div>;
